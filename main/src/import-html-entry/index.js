...
function getExecutableScript(scriptSrc, scriptText) {
  ...
  var proxy = opts.proxy,
    ...
    scopedGlobalVariables =
      _opts$scopedGlobalVar === void 0 ? [] : _opts$scopedGlobalVar;
  var sourceUrl = isInlineCode(scriptSrc)
    ? ""
    : "//# sourceURL=".concat(scriptSrc, "\n"); // 将 scopedGlobalVariables 拼接成函数声明，用于缓存全局变量，避免每次使用时都走一遍代理

  var scopedGlobalVariableFnParameters = scopedGlobalVariables.length
    ? scopedGlobalVariables.join(",")
  ...
  return strictGlobal
    ? scopedGlobalVariableFnParameters
      ? ";(function(){with(window.proxy){(function("
          .concat(scopedGlobalVariableFnParameters, "){;")
          .concat(scriptText, "\n")
      ...
      (scriptCache[scriptUrl] = fetch(scriptUrl)
        .then(function (response) {
          ...
          if (response.status >= 400) {
            ...
            throw new Error(
              ""
                .concat(scriptUrl, " load failed with status ")
...
function _execScripts(entry, scripts) {
  ...
  var _opts$fetch2 = opts.fetch,
    ...
    beforeExec =
      _opts$beforeExec === void 0 ? function () {} : _opts$beforeExec,
    ...
    scopedGlobalVariables =
      _opts$scopedGlobalVar2 === void 0 ? [] : _opts$scopedGlobalVar2;
    ...
    function exec(scriptSrc, inlineScript, resolve) {
      ...
      if (scriptSrc === entry) {
        ...
        try {
          ...
          console.error(
            "[import-html-entry]: error occurs while executing entry script ".concat(
              scriptSrc
            )
          );
          throw e;
        ...
        if (typeof inlineScript === "string") {
          try {
            ...
            //   e,
            //   "[import-html-entry]: error occurs while executing normal script ".concat(
            //     scriptSrc
            //   )
            // );
          }
          ...
          inlineScript.async &&
            (inlineScript === null || inlineScript === void 0
              ? void 0
              : inlineScript.content
                  .then(function (downloadedScriptText) {
                    return geval(inlineScript.src, downloadedScriptText);
                  })
                  ["catch"](function (e) {
                    throwNonBlockingError(
                      e,
                      "[import-html-entry]: error occurs while executing async script ".concat(
                        inlineScript.src
                      )
                    );
                  }));
...
export function importEntry(entry) {
  ...
  var _opts$fetch3 = opts.fetch,
    ...
    getTemplate =
      _opts$getTemplate === void 0 ? defaultGetTemplate : _opts$getTemplate,
  ...
  if (Array.isArray(entry.scripts) || Array.isArray(entry.styles)) {
    ...
    var getHTMLWithStylePlaceholder = function getHTMLWithStylePlaceholder(
      tpl
    ) {
    ...
    var getHTMLWithScriptPlaceholder = function getHTMLWithScriptPlaceholder(
      tpl
    ) {
